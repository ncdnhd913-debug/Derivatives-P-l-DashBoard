...
# Add the new validation logic for contract rate vs. start spot rate
    ...
    if uploaded_file is not None:
        try:
            ...
            required_columns_strict = ['회계일', '계정명', '차변', '대변', '환율']  # 기본 필수 열
            header_row = None
            ...
            # Re-map columns to the expected names (명시적 매핑)
            ...
            # Filter out rows that contain "월계" or "누계" in the '계정명' column
    ...
    if uploaded_file is None or df_ledger.empty:
        ...
        # 필수 열 확인: 거래환종
        if '거래환종' not in df_ledger.columns:
            st.error("업로드된 파일에 '거래환종' 열이 없습니다. 외화평가 환율(USD) 산출을 위해 필요합니다.")
        else:
            # 1) 거래환종이 USD인 행만 필터 (대소문자/공백 정규화)
            df_fx = df_ledger.copy()
            df_fx['거래환종_norm'] = df_fx['거래환종'].astype(str).str.upper().str.strip()
            df_fx = df_fx[df_fx['거래환종_norm'] == 'USD']

            # 2) 각 행의 '월말' 계산 및 "월말 날짜"와 정확히 일치하는 행만 유지
            #    - 회계일의 시간 성분 제거 후 비교
            df_fx['회계일_norm'] = df_fx['회계일'].dt.normalize()
            # Period('M') -> month end timestamp
            df_fx['월말'] = df_fx['회계일'].dt.to_period('M').dt.to_timestamp('M')
            df_fx = df_fx[df_fx['회계일_norm'] == df_fx['월말']]

            # 3) 월말에 여러 행이 있을 경우, 시간 순으로 마지막 기록을 사용
            df_fx = df_fx.sort_values('회계일').groupby('월말', as_index=False).last()

            # 4) 레이블을 "YYYY년 M월"로 생성 (0패딩 없음) 후 계약기간 내 월만 유지
            df_fx['회계연월'] = (
                df_fx['월말'].dt.year.astype(str) + '년 ' +
                df_fx['월말'].dt.month.astype(int).astype(str) + '월'
            )
            df_fx = df_fx[df_fx['회계연월'].isin(ordered_month_strings)]

            # 5) 외화평가 환율 시리즈 구성
            rates = df_fx[['회계연월', '환율']].copy()
            rates['환율 종류'] = '외화평가 환율'

            if rates.empty:
                st.warning("USD & 월말 조건을 만족하는 '외화평가 환율' 데이터를 찾지 못했습니다. 파일의 '거래환종'과 '회계일(월말)'을 확인해 주세요.")
            else:
                # 6) 계약환율과 외화평가 환율을 세로 결합
                df_rates_for_chart = pd.concat([df_contract_rate_data, rates], ignore_index=True)
    ...
    if not df_rates_for_chart.empty:
        ...
        # 차트 폭: 계약기간 길이에 따라 가변 (막대 차트와 일관)
        num_months = (end_date.year - start_date.year) * 12 + (end_date.month - start_date.month) + 1
        line_width = max(600, num_months * 80)
        ...
        line_chart = alt.Chart(df_rates_for_chart).mark_line(point=True).encode(
            ...
            title='계약환율 대비 외화평가 시점별 환율 변동 (USD, 월말 기준)',
            width=line_width,
            height=400
        ).interactive()
